...
class TestHostAgentInitialization:
    ...
    @patch('anvyl.agent.host_agent.get_infrastructure_client')
    def test_host_agent_init_with_defaults(self, mock_get_client):
        ...
        with patch.object(HostAgent, '_initialize_model', return_value=(Mock(), "test-model")):
            agent = HostAgent(
        ...
        assert agent.infrastructure_api_url == "http://localhost:4200"
        assert agent.port == 4201
        assert agent.model_provider_url == "http://localhost:11434/v1"
    ...
    def test_host_agent_init_with_custom_params(self, mock_get_client):
        ...
        with patch.object(HostAgent, '_initialize_model', return_value=(Mock(), "custom-model")):
            agent = HostAgent(
                communication=mock_communication,
                tools=mock_tools,
...
class TestHostAgentModelInitialization:
    ...
    def test_create_mock_model(self, mock_get_client):
        ...
        with patch.object(HostAgent, '_initialize_model') as mock_init:
            # Call the actual _create_mock_model method
            agent = HostAgent(communication=mock_communication, tools=[])
...
class TestHostAgentQueryProcessing:
    ...
    async def test_process_query_success(self, mock_get_client):
        ...
        with patch.object(HostAgent, '_initialize_model', return_value=(Mock(), "test-model")):
            host_agent = HostAgent(communication=mock_communication, tools=[])
...
class TestHostAgentInfo:
    ...
    def test_get_agent_info(self, mock_get_client):
        ...
        with patch.object(HostAgent, '_initialize_model', return_value=(Mock(), "llama-3.2")):
