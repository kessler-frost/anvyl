...
class TestInfrastructureServiceHostManagement:
    ...
    def test_list_hosts_success(self):
        ...
        mock_hosts = [
            Host(id="host1", name="Host 1", ip="192.168.1.100", os="macOS", status="online"),
...
class TestInfrastructureServiceContainerManagement:
    ...
    def test_stop_container_success(self):
        ...
        mock_db_container = Container(
            id="container-id", name="test", image="nginx:latest",
            host_id="host1", status="running"
        )
        self.service.db.get_container.return_value = mock_db_container
        self.service.db.update_container.return_value = None
        self.service.db.refresh_system_status.return_value = None
        
        result = self.service.stop_container("container-id", timeout=5)
        
        assert result is True
        mock_docker_container.stop.assert_called_once_with(timeout=5)
        self.service.db.update_container.assert_called_once()

    def test_stop_container_docker_unavailable(self):
        """Test stopping a container when Docker is unavailable."""
        self.service.docker_client = None
        
        result = self.service.stop_container("container-id")
...
class TestInfrastructureServiceErrorHandling:
    ...
    def test_stop_container_docker_error(self):
        ...
        result = self.service.stop_container("nonexistent-container")
